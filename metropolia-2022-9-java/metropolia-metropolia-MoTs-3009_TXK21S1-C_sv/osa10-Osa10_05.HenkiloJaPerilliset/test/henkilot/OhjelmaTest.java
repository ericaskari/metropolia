...
public class OhjelmaTest {
    ...
    public void tulostaLaitoksenHenkilotMetodiluokallaMain() throws Throwable {
        String klassName = "henkilot.HenkiloTulostus";
        Reflex.ClassRef<Object> classRef;
        classRef = Reflex.reflect(klassName);
        String metodi = "tulostaLaitoksenHenkilot";

        String virhe = "tee HenkiloTulostus-luokalle metodi public void tulostaLaitoksenHenkilot(List<Henkilo> henkilot)";

        assertTrue(virhe,
                classRef.method(metodi).returningVoid().taking(List.class).isPublic());

        String v = "List<Henkilo> lista = new ArrayList<Henkilo>();\n"
                + "lista.add( new Opiskelija(\"Pekka\",\"Korso\") );\n"
                + "new HenkiloTulostus().tulostaLaitoksenHenkilot(lista)\n";

        List lista = new ArrayList();
        lista.add(newOpiskelija("Pekka", "Korso"));

        classRef.method(metodi).returningVoid().taking(List.class).withNiceError().invokeOn(classRef.ctor().takingNoParams().invoke(), lista);
    }
