...
class ShowModel {
    ...
    static fromApi(show: Partial<ShowModel>): ShowModel {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const searchInputEl = document.querySelector<HTMLButtonElement>('#search-input');
    const searchResultEl = document.querySelector<HTMLButtonElement>('#search-result');
    ...
    const updateUI = (models: ShowModel[]) => {
        if (models.length === 0) {
        ...
        const htmlElements: HTMLDivElement[] = models.map((show) => {
    ...
    const getDataFromEndpoint = async (searchText: string) => {
        try {
            isOngoingRequest = true;

            showLoadingSpinner();

            const response: SearchShowResponse[] = await fetch(`https://api.tvmaze.com/search/shows?q=${searchText}`, { signal }).then(response => response.json());

            isOngoingRequest = false;

            console.log(response);

            const models: ShowModel[] = response.map(x => ShowModel.fromApi(x.show));
