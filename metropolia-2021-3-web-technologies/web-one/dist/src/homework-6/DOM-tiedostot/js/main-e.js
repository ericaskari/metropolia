...
function imageBuilder(imgSource, altText) {
...
function figureBuilder(imgSource, altText, captionText) {
    const figure = document.createElement('figure');

    figure.appendChild(imageBuilder(imgSource, altText));

    const figureCaption = document.createElement('figcaption');

    figureCaption.innerText = captionText;
...
function articleHeaderBuilder(headerText) {
...
function articleDescriptionBuilder(descriptionText) {
...
function articleBuilder(headerText, descriptionText, imgSource, altText, captionText) {
    const article = document.createElement('article');

    article.appendChild(articleHeaderBuilder(headerText));

    article.appendChild(figureBuilder(imgSource, altText, captionText));

    article.appendChild(articleDescriptionBuilder(descriptionText));
...
document.addEventListener("DOMContentLoaded", () => {
...

    {
        ...
        picArray
            .map((image) => articleBuilder(image.title, image.description, image.filename, image.caption, image.caption))
            .forEach((article) => main.appendChild(article));
    }
});
